<?php
	...
	while(true) {
		...
		$result = __get_race_results_for_scratch();

		foreach ($result as $event_obj) {
			if(!(isset($event_obj->venue_id))) continue;
			if(is_null($event_obj->venue_name)) continue;
			if($event_obj->type == "time") {
				...
				__api_send2__("http://staging.dw.xtradeiom.com/api/meetings", "POST", $event, "nar2_time");
			} else {
				$event = new \stdClass();
				$event->timestamp = time();
				$event->venue_id = $event_obj->venue_id;
				...
				__api_send2__("http://staging.dw.xtradeiom.com/api/meetings", "POST", $event, "nar2_".$event_type, $log_event);
			}
		}
		sleep(3);
	}	
