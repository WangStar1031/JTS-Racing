...
class CustomPDF extends FPDF
{
	...
	function Header()
	{
		$this->Image("app/img/vision-logo.jpg", 10, 10, 8);
	}
...
if($c == 1){ // Check Race Time Change
	...
	echo $htmlBuff;
	$pdfName = "jrastewards/" . date("F jS, Y", strtotime($date_value))." JRA Stewards Reports";
	...
	echo ' <a href="'.$link_href.'" style="text-decoration: none; color: green;"> ( '.(($lang == "en")?"English":"日本語").' ) </a>';
	if($lang == "en") {
		...
		echo "<a class='btnForExport pdfHiden' href='".$pdfName."_".$lang.".pdf' download>Export PDF</a>";
...
<style>
	...
	* {
	...
	foreach ($arr as $event) {
		...
		if($event_string){
			...
			if($meeting_id !== $event->meeting_id) {
				...
				$pdf->SetTextColor(80,80,80);
				$pdf->Write( 6, $meeting_name_printing[0] . " - " . processDate($date_str) . " - " . $meeting_name_printing[1] .  " Meeting " . trim($arr4Day[1]));
	...
	$pdf->Output( 'F', $pdfName . '_' . $lang .'.pdf');
