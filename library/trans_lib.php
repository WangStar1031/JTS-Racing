<?php
	...
	require __DIR__ . '/google_free/vendor/autoload.php';
	...
	function get_word_patterns($offline = true) {
		...
		$patterns = $db->select("select * from tbl_words order by jpn");
		return $patterns;
	}
	...
	function get_sentence_patterns($offline = true) {
		...
		$sentences = $db->select("select * from tbl_sentences order by jpn");
		return $sentences;
	}
	...
	function post_pattern_process($sentence, $patterns, $details) {
		...
		for ($i=0; $i<count($patterns); $i++) {
			...
			$pos_info->word_status = $pattern_obj["Id"];
		...
		for($i = count($arr_pos)-1; $i>=0; $i--) {
			...
			$sentence = $sentence_01.($pos_obj->word_status == '0' ? ' <img src="./app/img/google.png" class="zeroStatus">' : '').'<span class="sentence_pattern" pattern_id="'.$id.'" pattern_source="'.$pattern_source.'">'.$sentence_02.'</span>'.$sentence_03;
		}
		return $sentence;
	}
			...
			foreach ($horse_arr as $key => $horse_obj) {
				...
				if(count($horse_obj_detail) != 2) 
					...
					if(strpos($horse_obj_detail[1], "の騎手") !== false){
						$horse_arr[$key] = $horse_eng_name."'s jockey ".__kata2romaji__(__get_values($horse_obj_detail[1]."ののの", "の騎手", "ののの"));
