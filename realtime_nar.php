	...
	while(true) {
		...
		foreach ($result as $event_obj) {
			...
			if($event_obj->type == "time") {
				...
				__api_send2__("http://staging.dw.xtradeiom.com/api/meetings", "POST", $event, "nar_time");
			} else {
				$event = new \stdClass();
				$event->timestamp = time();
				$event->venue_id = $event_obj->venue_id;
				...
				__api_send2__("http://staging.dw.xtradeiom.com/api/meetings", "POST", $event, "nar_".$event_type, $log_event);
			}
		}
		sleep(3);
	}	
